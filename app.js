...
// Speech Recognition Engine - Mobile Forced Real-time Paint
...
async function startStream() {
    ...
    try {
        ...
        // Prevents ringing/feedback sound by NOT connecting to audioCtx.destination
...
// PWA Install Prompt & Dynamic Button Auto-Hide Logic
let deferredPrompt;
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // Show the custom install button if app is not yet installed
    if (installAppBtn) {
...
if (installAppBtn) {
    installAppBtn.addEventListener('click', async () => {
        ...
        if (outcome === 'accepted') {
            ...
            installAppBtn.style.display = 'none'; // Hide button immediately after install accept
        }
        deferredPrompt = null;
    });
}

// Automatically hide the install button if the app is already running in standalone PWA mode or successfully installed
...
// Check on load if app is running in standalone mode (already installed)
